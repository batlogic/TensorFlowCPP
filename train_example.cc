...
namespace tf {
    ...
    tf::Output FullyConnectedLinear(const tf::Scope& scope, tf::Input inp, const int sizeOutput) {
        ...
        tf::Assign(scope, weights, tf::TruncatedNormal(scope, weights, tf_dt));
        auto biases  = tf::Variable(scope, {sizeOutput}, tf_dt);
        tf::Assign(scope, biases, tf::ZerosLike(scope, biases));
...
int main(int argc, char* argv[]) {
    ...
    auto fc1 = tf::FullyConnectedLinear(scope,inputLinesPlaceHolder,nhidden);

    // activation of layer 1
    auto tanh1   = tf::Tanh(scope,fc1);

    // Layer 2: Fully Connected. Input = nhidden. Output = nout.
    auto fc2 = tf::FullyConnectedLinear(scope,tanh1,nout);
